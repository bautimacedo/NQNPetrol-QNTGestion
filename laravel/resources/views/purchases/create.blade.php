...
<div class="space-y-6">
    <div>
        <h2 class="text-3xl font-bold text-gray-100">Nueva Compra</h2>
        <p class="mt-2 text-gray-400">Crear una nueva compra en el sistema</p>
    </div>
        ...
        <form action="{{ route('purchases.store') }}" method="POST">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <textarea name="description" required rows="3" class="w-full px-3 py-2 rounded-lg border border-gray-600 bg-gray-700 text-gray-100 focus:outline-none focus:ring-2 focus:ring-teal-500">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="total_amount" step="0.01" min="0" required value="{{ old('total_amount') }}" class="w-full px-3 py-2 rounded-lg border border-gray-600 bg-gray-700 text-gray-100 focus:outline-none focus:ring-2 focus:ring-teal-500">
                        @error('total_amount')
                            <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div class="flex justify-end space-x-3 mt-6">
                <a href="{{ route('purchases.index') }}" class="px-4 py-2 text-sm font-medium rounded-lg transition-colors text-gray-300 hover:text-white" style="border: 1px solid rgba(255, 255, 255, 0.2);">
                    Cancelar
                </a>
