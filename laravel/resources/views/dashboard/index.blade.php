...
@section('page-subtitle', 'Vista general de la flota y operaciones en tiempo real')
...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            @foreach($batteriesHighFlightCount as $battery)
                <div class="bg-gray-50 rounded-lg border border-amber-200 p-4">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="font-bold text-gray-900">{{ $battery->serial }}</p>
                            <p class="text-xs text-gray-600 mt-1">{{ $battery->drone->name ?? 'Sin asignar' }}</p>
                        </div>
                        <div class="text-right">
                            <p class="text-lg font-bold text-amber-600">{{ $battery->flight_count }}</p>
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div class="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div class="flex items-center justify-between mb-6">
                ...
                <span class="px-3 py-1 text-xs font-bold bg-red-100 text-red-700 rounded-full border border-red-200">
                ...
                <div class="space-y-3 max-h-[400px] overflow-y-auto pr-2">
                    @foreach($expiringLicenses as $license)
                        <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg border border-gray-200 hover:bg-gray-100 transition-colors">
                            <div>
                                <p class="font-bold text-gray-900">{{ $license->authorizedUser->full_name ?? 'N/A' }}</p>
                                <p class="text-xs text-gray-600 mt-1 uppercase tracking-wider">{{ $license->type }}</p>
                            </div>
                            <div class="text-right">
            ...
            @else
                <div class="flex flex-col items-center justify-center py-12 text-center">
                    ...
                    <p class="text-gray-600">Todo en regla. No hay licencias críticas.</p>
                </div>
            @endif
        </div>
        ...
        <div class="bg-white rounded-xl shadow-md border border-gray-200 p-6">
            <div class="flex items-center justify-between mb-6">
                <h3 class="text-lg font-semibold text-gray-900">Condiciones Climáticas</h3>
            ...
            <div class="space-y-5">
                <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    <div class="flex items-center">
                        <div class="w-8 h-8 flex items-center justify-center bg-orange-100 rounded mr-3">
                            <svg class="w-5 h-5 text-orange-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"></path></svg>
                        </div>
                        <span class="text-sm text-gray-700">Temperatura</span>
                    </div>
                    <span class="text-xl font-bold text-gray-900">{{ number_format($weather['temperature'] ?? 0, 1) }}°C</span>
                </div>
                
                <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg border-l-4 {{ ($weather['wind_speed'] ?? 0) >= 35 ? 'border-red-500' : 'border-green-500' }}">
                    <div class="flex items-center">
                        <div class="w-8 h-8 flex items-center justify-center bg-blue-100 rounded mr-3">
                        ...
                        <span class="text-sm text-gray-700">Viento Sostenido</span>
                    </div>
                    <span class="text-xl font-bold {{ ($weather['wind_speed'] ?? 0) >= 35 ? 'text-red-600' : 'text-green-600' }}">
                ...
                <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    <div class="flex items-center">
                        <div class="w-8 h-8 flex items-center justify-center bg-purple-100 rounded mr-3">
                        ...
                        <span class="text-sm text-gray-700">Ráfagas</span>
                    </div>
                    <span class="text-xl font-bold {{ ($weather['wind_gusts'] ?? 0) >= 45 ? 'text-red-600' : 'text-purple-600' }}">
                ...
                <div class="pt-4 border-t border-gray-200">
                    ...
                    <div class="flex items-center">
                        <p class="text-base font-medium text-gray-900 capitalize">{{ $weather['description'] ?? 'Sin datos' }}</p>
                    </div>
                ...
                @if(isset($weather['last_update']) && $weather['last_update'] === 'N/A')
                    <div class="pt-2 px-2 py-1 bg-amber-50 border border-amber-200 rounded">
                        <p class="text-[10px] text-amber-700 font-bold uppercase text-center">Datos no disponibles</p>
                    </div>
                @endif
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            @endphp
            <div class="flex items-center p-4 {{ $isSafeToFly ? 'bg-green-50' : 'bg-red-50' }} rounded-xl border {{ $isSafeToFly ? 'border-green-200' : 'border-red-200' }}">
                <div class="w-4 h-4 {{ $isSafeToFly ? 'bg-green-500' : 'bg-red-500' }} rounded-full animate-pulse mr-3"></div>
                <div>
                    <span class="text-lg font-bold {{ $isSafeToFly ? 'text-green-700' : 'text-red-700' }}">
