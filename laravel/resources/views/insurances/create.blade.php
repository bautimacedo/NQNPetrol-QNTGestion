...
@section('page-title', 'Nuevo Seguro')
@section('page-subtitle', 'Registrar un nuevo seguro')
    ...
    <form action="{{ route('insurances.store') }}" method="POST">
        ...
        <div class="space-y-4">
            <div>
                ...
                <input type="text" name="insurer_name" required value="{{ old('insurer_name') }}" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#6b7b39] focus:border-[#6b7b39] transition-colors" placeholder="Ej: Seguros Generales S.A.">
                @error('insurer_name')
            ...
            <div>
                ...
                <input type="text" name="policy_number" required value="{{ old('policy_number') }}" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#6b7b39] focus:border-[#6b7b39] transition-colors">
                @error('policy_number')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="date" name="validity_date" required value="{{ old('validity_date') }}" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#6b7b39] focus:border-[#6b7b39] transition-colors">
                @error('validity_date')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
