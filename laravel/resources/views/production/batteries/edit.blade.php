...
@section('page-title', 'Editar Batería')
@section('page-subtitle', 'Modificar información de la batería')

@section('content')
<div class="bg-white rounded-xl shadow-md border border-gray-200 p-6 max-w-2xl">
    <form action="{{ route('production.batteries.update', $battery) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-6">
            <div>
                ...
                <input type="text" name="serial" value="{{ old('serial', $battery->serial) }}" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#6b7b39] focus:border-[#6b7b39] transition-colors">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="flight_count" value="{{ old('flight_count', $battery->flight_count) }}" min="0" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#6b7b39] focus:border-[#6b7b39] transition-colors">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Último Uso</label>
                    <input type="date" name="last_used" value="{{ old('last_used', $battery->last_used ? $battery->last_used->format('Y-m-d') : '') }}" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#6b7b39] focus:border-[#6b7b39] transition-colors">
                </div>
            </div>
