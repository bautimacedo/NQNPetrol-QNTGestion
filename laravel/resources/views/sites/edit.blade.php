...
@section('page-subtitle', 'Modificar información de la ubicación')
    ...
    <form action="{{ route('sites.update', $site) }}" method="POST">
        ...
        <div class="space-y-6">
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $site->name) }}" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#6b7b39] focus:border-[#6b7b39] transition-colors">
                @error('name')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
            </div>

            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Detalles de Ubicación</label>
                <textarea name="location_details" rows="3" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#6b7b39] focus:border-[#6b7b39] transition-colors">{{ old('location_details', $site->location_details) }}</textarea>
